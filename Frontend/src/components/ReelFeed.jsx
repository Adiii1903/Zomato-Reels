...
const ReelFeed = ({ items = [], onLike, onSave, emptyMessage = 'No videos yet.', isLoading = false, onEndReached }) => {
  ...
  useEffect(() => {
    ...
    if (!activeId || !indexById.has(activeId)) {
      const initialIds = items.slice(0, 4).map((item) => item._id)
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ...
      { root: feedRef.current ?? null, threshold: [0, 0.25, 0.6, 0.9, 1] }
    )
  ...
  useEffect(() => {
    ...
    const nextIds = items.slice(Math.max(index - 1, 0), index + 3).map((item) => item._id)
    setLoadedIds((prev) => {
      const next = new Set(prev)
